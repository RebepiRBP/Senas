    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        if ((window as any).Hands) {
          ...
          reject(new Error('MediaPipe Hands constructor not available'))
        }
      }
  ...
  useEffect(() => {
    const initializeHands = async () => {
      try {
        ...
        const HandsConstructor = await loadMediaPipe()
        ...
        setError('Error al cargar MediaPipe desde CDN. Verifica tu conexión a internet.')
        setIsLoading(false)
      }
    }
